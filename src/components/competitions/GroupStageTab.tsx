...
const ErrorDisplay = ({
	...
	<div className="flex flex-col items-center justify-center min-h-[300px] text-center px-6">
		...
		{onRetry && (
			<button
				onClick={onRetry}
				className="inline-flex items-center gap-2 bg-green-900 
                text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-green-800 
			...
			>
				...
				>
					<path
                        ...
                        0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"
					/>
				</svg>
				Try Again
			</button>
...
const RankBadge = ({ rank, total }: { rank: number; total: number }) => {
	...
	if (rank === 1)
		return (
			<span className={`${badgeBase} bg-yellow-400 text-yellow-900`}>
				{rank}
			</span>
		);
	if (rank === 2)
		return (
			<span className={`${badgeBase} bg-gray-300 text-gray-700`}>{rank}</span>
		);
	if (rank === total)
		return (
			<span className={`${badgeBase} bg-red-100 text-red-500`}>{rank}</span>
		);
	return (
					...
					<table className="min-w-full">
						<thead>
							<tr className="border-b border-gray-100">
                                ...
                                text-[10px] font-bold text-gray-400 uppercase tracking-widest w-8">
									#
								</th>
								<th className="px-2 py-2.5 text-left text-[10px] font-bold 
								...
								return (
									...
									>
										...
										{[
											...
											<td
												key={i}
												className="px-2 py-3 text-center text-xs tabular-nums text-gray-500"
											>
												{val}
											</td>
										))}
										<td className="px-2 py-3 text-center">
                                            ...
                                            justify-center min-w-[1.75rem] px-1.5 py-0.5 
                                            rounded-md bg-green-900 text-white text-xs 
                                            font-bold tabular-nums shadow-sm">
												{s.points}
											</span>
										</td>
									</tr>
