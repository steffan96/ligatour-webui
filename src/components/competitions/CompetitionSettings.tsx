...
const StatusBadge = ({ status }: { status: string }) => {
	...
	return (
		<span
			className={`text-xs font-bold px-2.5 py-1 rounded-md border ${styles[status] ?? styles.inactive}`}
		>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
				<Field label="Competition Type">
					<div className={inputCls(true)}>
						{CompetitionTypeDisplay[
							draft.type as keyof typeof CompetitionTypeDisplay
						] ?? "—"}
					</div>
				</Field>
			</div>
