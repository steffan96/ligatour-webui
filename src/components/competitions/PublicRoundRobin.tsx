...
const RankBadge = ({ rank }: { rank: number }) => {
	if (rank === 1)
		return (
			<span className={`${badgeBase} bg-yellow-400 text-yellow-900`}>1</span>
		);
	if (rank === 2)
		return <span className={`${badgeBase} bg-gray-300 text-gray-700`}>2</span>;
	if (rank === 3)
		return (
			<span className={`${badgeBase} bg-orange-300 text-orange-800`}>3</span>
		);
	return (
...
const StatCell = ({ value }: { value: number | string }) => (
	<td className="px-4 py-3.5 text-sm text-center tabular-nums text-gray-600">
		{value}
	</td>
);
					...
					{standings.map((standing, index) => {
						...
						return (
							...
							>
								...
								<td className="px-4 py-3.5">
									<span
										className={`text-sm font-semibold ${isTop3 ? "text-green-900" : "text-gray-800"}`}
									>
...
const PublicRoundRobin = () => {
	...
	const fetchData = async () => {
		...
		try {
			...
			const { competition: competitionData, standings: standingsData } =
				response.data;
			...
			setError(
				err ||
					"Failed to load competition. The link may be invalid or expired.",
			);
		} finally {
			setLoading(false);
		}
				...
				<div className="w-full px-4 sm:px-6 lg:px-8 py-5">
					<div className="flex items-center justify-between gap-4 flex-wrap">
						<div className="flex items-center gap-3">
							<div
								className={
									"flex-shrink-0 w-10 h-10 bg-green-900 rounded-lg " +
