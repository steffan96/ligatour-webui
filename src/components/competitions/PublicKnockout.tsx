...
const TeamRow = ({ team, score, penalties, isWinner, isLoser, isWalkover }: TeamRowProps) => (
  <div className={['flex items-center justify-between px-3 min-h-[38px]', isWinner ? 'bg-green-50' : ''].join(' ')}>
    <span
      className={[
...
const MatchCard = ({ match }: { match: KnockoutMatch }) => {
  ...
  return (
    <div
      style={{ height: CARD_H }}
      className={[
        'rounded-lg border overflow-hidden flex flex-col select-none transition-all duration-150',
        isLive
    ...
    >
      {(isLive || isWalkover || isBye) && (
        ...
        >
          {isLive && <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse flex-shrink-0" />}
...
const PublicKnockout = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useCompetitionData<KnockoutRound[]>(
    slug,
    'knockout',
