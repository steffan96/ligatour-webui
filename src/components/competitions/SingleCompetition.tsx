import { type CompetitionInterface, getCompetition } from 'api/competitions';
...
const PublicLink = ({ link, onCopy }: { link: string; onCopy: (text: string) => void }) => (
  <div>
...
const SingleCompetition = () => {
  ...
  const headerActionButton = useMemo(() => {
    if (selectedRoundId !== null) return <BackButton label="Back to Rounds" onClick={() => setSelectedRoundId(null)} />;
    if (selectedTeamId !== null) return <BackButton label="Back to Teams" onClick={() => setSelectedTeamId(null)} />;
  ...
  return (
    <PageWindow title={`${competition.name}`} headerActionButtons={headerActionButton}>
      <TabBar activeTab={activeTab} onTabChange={handleTabChange} isIndividual={competition.individual} />

      {activeTab === 'overview' && <CompetitionOverview competition={competition} onCopy={handleCopyLink} />}
      ...
      {activeTab === 'rounds' &&
        (selectedRoundId === null ? (
          ...
          <SingleRound competitionId={competition.id} roundId={selectedRoundId} />
        ))}
