...
const InlineForm = ({
	...
	return (
		...
		>
			...
			<div
				className={`flex justify-end gap-3 pt-3 mt-3 border-t ${g ? "border-green-200" : "border-blue-200"}`}
			>
				<button
					onClick={onCancel}
...
const Teams = ({ competition, onManagePlayers }: TeamsProps) => {
	...
	const handleAddTeam = async () => {
		...
		try {
			const response = await addTeamToCompetition(
		...
		<div className="space-y-5">
			...
			{editingId !== null && editingParticipant && (
				<InlineForm
					...
					onChange={(v) =>
						setEditingParticipant({ ...editingParticipant, name: v })
					}
