...
export const restartMatch = async (competitionId: number, roundId: number, matchId: number) => {
	try {
		const response = await axiosInstance.put(`/api/v1/matches/${competitionId}/${roundId}/${matchId}/restart`);
